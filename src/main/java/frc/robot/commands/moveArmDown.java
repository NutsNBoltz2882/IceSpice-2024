...
public class moveArmDown extends Command {
    ...
    public moveArmDown(Intake subsystem) {
        m_intake = subsystem;
    ...
    public void initialize() {
        ...
        // Initialize the targetPosition variable
        if(currentPos != 70){
            /*if the position isn't what we want is not 70 its going to be
             70 no matter what it is when command first runs*/
            targetPosition = 70;
    ...
    public void execute() {
        ...
        double error = targetPosition - Math.round(currentPos);
        ...
        if (Math.abs(error) > 1.0) { 
            // Check if the error is greater than 1 degree to avoid constant movement
            ...
            m_intake.stopArm();
            m_intake.armStatus(!m_intake.armStatus);
    ...
    public void end(boolean interrupted) {
        //tells if the command ended due to an interruption
        SmartDashboard.putBoolean("Arm Down was Interrupted", interrupted);
    }
